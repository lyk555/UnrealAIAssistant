...
/**
 * Main Codex chat widget for the editor
 */
class UNREALAIASSISTANT_API SAssistantEditorWidget : public SCompoundWidget
{
public:
	SLATE_BEGIN_ARGS(SAssistantEditorWidget)
	{}
	SLATE_END_ARGS()

	void Construct(const FArguments& InArgs);
	virtual ~SAssistantEditorWidget();
...
private:
	...
	/** Send the current input to Codex */
	void SendMessage();
	...
	/** Handle response from Codex */
	void OnCodexResponse(const FString& Response, bool bSuccess);
...
private:
	...
	/** Input area widget */
	TSharedPtr<SAssistantInputArea> InputArea;
	...
	/** Include UE5.5 context in prompts */
	bool bIncludeUE55Context = true;

	/** Include project context in prompts */
	bool bIncludeProjectContext = true;

	/** Handle streaming progress from Codex (legacy, still used for accumulation) */
	void OnCodexProgress(const FString& PartialOutput);

	/** Handle structured NDJSON stream events from Codex */
	void OnCodexStreamEvent(const FCodexStreamEvent& Event);
	...
	void HandleToolUseEvent(const FCodexStreamEvent& Event);

	/** Handle a ToolResult stream event (update tool indicator with completion + result) */
	void HandleToolResultEvent(const FCodexStreamEvent& Event);

	/** Handle a Result stream event (append stats footer) */
	void HandleResultEvent(const FCodexStreamEvent& Event);
