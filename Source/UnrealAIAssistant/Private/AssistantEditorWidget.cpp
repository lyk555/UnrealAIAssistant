...
#include "Widgets/SAssistantToolbar.h"
#include "Widgets/SAssistantInputArea.h"
...
// ============================================================================
// SAssistantEditorWidget
// ============================================================================

void SAssistantEditorWidget::Construct(const FArguments& InArgs)
{
	ChildSlot
	[
		SNew(SVerticalBox)
	...
	FCodexCodeSubsystem& Subsystem = FCodexCodeSubsystem::Get();
	if (!Subsystem.HasAnyAvailableProvider())
	{
		AddMessage(TEXT("No supported AI backend was found.\n\nInstall a supported CLI backend and reopen assistant."), false);
	}
	else
	{
		FString WelcomeMessage = FString::Printf(
			TEXT("assistant is ready.\n\nCurrent backend: %s\n\nAsk about UE5.5 code patterns, Blueprints, engine systems, debugging, and editor automation.\n\n"),
			*Subsystem.GetActiveProviderDisplayName());
...
SAssistantEditorWidget::~SAssistantEditorWidget()
{
	// Cancel any pending requests
	FCodexCodeSubsystem::Get().CancelCurrentRequest();
}

TSharedRef<SWidget> SAssistantEditorWidget::BuildToolbar()
{
	return SNew(SAssistantToolbar)
		.bUE55ContextEnabled_Lambda([this]() { return bIncludeUE55Context; })
		.bProjectContextEnabled_Lambda([this]() { return bIncludeProjectContext; })
		.bRestoreEnabled_Lambda([this]() { return FCodexCodeSubsystem::Get().HasSavedSession(); })
		.CurrentProvider_Lambda([]() { return FCodexCodeSubsystem::Get().GetActiveProvider(); })
		.OnProviderChanged_Lambda([this](ECodexProvider Provider)
		{
			FCodexCodeSubsystem::Get().SetActiveProvider(Provider);
			AddMessage(FString::Printf(TEXT("Active backend set to %s."), *FCodexCodeSubsystem::Get().GetActiveProviderDisplayName()), false);
		})
		.OnUE55ContextChanged_Lambda([this](bool bEnabled) { bIncludeUE55Context = bEnabled; })
...
TSharedRef<SWidget> SAssistantEditorWidget::BuildChatArea()
{
	return SNew(SBorder)
		.BorderImage(FAppStyle::GetBrush("ToolPanel.DarkGroupBorder"))
		.Padding(4.0f)
...
TSharedRef<SWidget> SAssistantEditorWidget::BuildInputArea()
{
	SAssignNew(InputArea, SAssistantInputArea)
...
TSharedRef<SWidget> SAssistantEditorWidget::BuildStatusBar()
{
	return SNew(SBorder)
		...
		[
			...
			[
				SNew(STextBlock)
				.Text(this, &SAssistantEditorWidget::GetStatusText)
				.ColorAndOpacity(this, &SAssistantEditorWidget::GetStatusColor)
			]
...
void SAssistantEditorWidget::AddMessage(const FString& Message, bool bIsUser)
{
...
void SAssistantEditorWidget::SendMessage()
{
	...
	if (!IsCodexAvailable())
	{
		AddMessage(FString::Printf(TEXT("The %s backend is not available. Install the CLI and try again."), *FCodexCodeSubsystem::Get().GetActiveProviderDisplayName()), false);
		return;
	}
	...
	// Send to Codex using FCodexPromptOptions
	FOnCodexResponse OnComplete;
	OnComplete.BindSP(this, &SAssistantEditorWidget::OnCodexResponse);
	...
	Options.OnProgress.BindSP(this, &SAssistantEditorWidget::OnCodexProgress);
	Options.OnStreamEvent.BindSP(this, &SAssistantEditorWidget::OnCodexStreamEvent);
	Options.AttachedImagePaths = ImagePaths;

	FCodexCodeSubsystem::Get().SendPrompt(Prompt, OnComplete, Options);
}

void SAssistantEditorWidget::OnCodexResponse(const FString& Response, bool bSuccess)
{
...
void SAssistantEditorWidget::ClearChat()
{
	if (ChatMessagesBox.IsValid())
	{
	...
	FCodexCodeSubsystem::Get().ClearHistory();
	LastResponse.Empty();
	ResetStreamingState();
...
void SAssistantEditorWidget::CancelRequest()
{
	FCodexCodeSubsystem::Get().CancelCurrentRequest();
	bIsWaitingForResponse = false;
...
void SAssistantEditorWidget::CopyToClipboard()
{
	if (!LastResponse.IsEmpty())
	{
		FPlatformApplicationMisc::ClipboardCopy(*LastResponse);
		UE_LOG(LogUnrealAIAssistant, Log, TEXT("Copied response to clipboard"));
	}
}

void SAssistantEditorWidget::RestoreSession()
{
	FCodexCodeSubsystem& Subsystem = FCodexCodeSubsystem::Get();
...
void SAssistantEditorWidget::NewSession()
{
...
bool SAssistantEditorWidget::IsCodexAvailable() const
{
	return FCodexCodeSubsystem::Get().IsActiveProviderAvailable();
}

FText SAssistantEditorWidget::GetStatusText() const
{
	if (bIsWaitingForResponse)
	{
		double ElapsedSec = FPlatformTime::Seconds() - StreamingStartTime;
		FString StatusStr = FString::Printf(TEXT("assistant is thinking... %.1fs"), ElapsedSec);
	...
	if (!IsCodexAvailable())
	{
		return FText::FromString(FString::Printf(TEXT("assistant unavailable (%s)"), *FCodexCodeSubsystem::Get().GetActiveProviderDisplayName()));
	}

	if (!LastResultStats.IsEmpty())
	{
		return FText::FromString(LastResultStats);
	}

	return FText::FromString(FString::Printf(TEXT("assistant ready (%s)"), *FCodexCodeSubsystem::Get().GetActiveProviderDisplayName()));
}

FSlateColor SAssistantEditorWidget::GetStatusColor() const
{
	if (bIsWaitingForResponse)
	{
...
void SAssistantEditorWidget::ResetStreamingState()
{
	StreamingResponse.Empty();
...
void SAssistantEditorWidget::StartStreamingResponse()
{
	ResetStreamingState();
	...
	if (ChatMessagesBox.IsValid())
	{
		...
		[
			SNew(STextBlock)
			.Text_Lambda([]() { return FText::FromString(TEXT("assistant")); })
			.TextStyle(FAppStyle::Get(), "SmallText")
		...
		[
			...
			[
				...
				.Text(FText::FromString(TEXT("assistant is thinking...")))
				.TextStyle(FAppStyle::Get(), "NormalText")
...
void SAssistantEditorWidget::OnCodexProgress(const FString& PartialOutput)
{
...
void SAssistantEditorWidget::OnCodexStreamEvent(const FCodexStreamEvent& Event)
{
	switch (Event.Type)
	{
	case ECodexStreamEventType::SessionInit:
		UE_LOG(LogUnrealAIAssistant, Log, TEXT("[StreamEvent] SessionInit: session_id=%s"), *Event.SessionId);
		break;

	case ECodexStreamEventType::TextContent:
		UE_LOG(LogUnrealAIAssistant, Log, TEXT("[StreamEvent] TextContent: %d chars"), Event.Text.Len());
		break;

	case ECodexStreamEventType::ToolUse:
		UE_LOG(LogUnrealAIAssistant, Log, TEXT("[StreamEvent] ToolUse: %s (id=%s)"), *Event.ToolName, *Event.ToolCallId);
		HandleToolUseEvent(Event);
		break;

	case ECodexStreamEventType::ToolResult:
		UE_LOG(LogUnrealAIAssistant, Log, TEXT("[StreamEvent] ToolResult: tool_id=%s, %d chars"),
			*Event.ToolCallId, Event.ToolResultContent.Len());
		HandleToolResultEvent(Event);
		break;

	case ECodexStreamEventType::Result:
		UE_LOG(LogUnrealAIAssistant, Log, TEXT("[StreamEvent] Result: error=%d, duration=%dms, turns=%d, cost=$%.4f"),
			Event.bIsError, Event.DurationMs, Event.NumTurns, Event.TotalCostUsd);
		HandleResultEvent(Event);
		break;

	default:
		UE_LOG(LogUnrealAIAssistant, Log, TEXT("[StreamEvent] Unknown type: %d"), static_cast<int32>(Event.Type));
		break;
	}
}

void SAssistantEditorWidget::FinalizeStreamingResponse()
{
	...
	// Clear all streaming state (except StreamingResponse which is used by OnCodexResponse)
	StreamingTextBlock.Reset();
	StreamingContentBox.Reset();
	CurrentSegmentText.Empty();
	ToolCallStatusLabels.Empty();
...
void SAssistantEditorWidget::HandleToolUseEvent(const FCodexStreamEvent& Event)
{
...
void SAssistantEditorWidget::HandleToolResultEvent(const FCodexStreamEvent& Event)
{
	// Look up tool name
...
void SAssistantEditorWidget::HandleResultEvent(const FCodexStreamEvent& Event)
{
	if (!StreamingContentBox.IsValid())
	{
		return;
...
FString SAssistantEditorWidget::GetDisplayToolName(const FString& FullToolName)
{
	FString Name = FullToolName;
	// Strip common MCP server prefix for cleaner display
	Name.RemoveFromStart(TEXT("mcp__UnrealAIAssistant__unreal_"));
	Name.RemoveFromStart(TEXT("mcp__unrealcodex__unreal_"));
	Name.RemoveFromStart(TEXT("mcp__unrealclaude__unreal_"));
	return Name;
}

void SAssistantEditorWidget::UpdateToolGroupSummary()
{
...
void SAssistantEditorWidget::ParseCodeFences(const FString& Input, TArray<TPair<FString, bool>>& OutSections)
{
	OutSections.Empty();
...
void SAssistantEditorWidget::ParseAndRenderCodeBlocks()
{
...
void SAssistantEditorWidget::AppendToLastResponse(const FString& Text)
{
	// Delegate to OnCodexProgress for streaming updates
	OnCodexProgress(Text);
}

void SAssistantEditorWidget::RefreshProjectContext()
{
...
FText SAssistantEditorWidget::GetProjectContextSummary() const
{
...
FString SAssistantEditorWidget::GenerateMCPStatusMessage() const
{
	...
	if (!FUnrealAIAssistantModule::IsAvailable())
	{
	...
	TSharedPtr<FUnrealAIAssistantMCPServer> MCPServer = FUnrealAIAssistantModule::Get().GetMCPServer();
	...
	const TArray<FString>& ExpectedTools = UnrealAIAssistantConstants::MCPServer::ExpectedTools;
