	...
	, ScriptCounter(0)
{
	...
	if (!IFileManager::Get().DirectoryExists(*ContentScriptDir))
	{
		...
		UE_LOG(LogUnrealAIAssistant, Log, TEXT("Created script directory: %s"), *ContentScriptDir);
	}
}
	...
	const FString& Description)
{
	...
	UE_LOG(LogUnrealAIAssistant, Log, TEXT("C++ script written to: %s"), *FilePath);
	...
	// Compilation failed - return error for Codex to fix and retry
	// Codex will call execute_script again with fixed code
	UE_LOG(LogUnrealAIAssistant, Warning, TEXT("C++ compilation failed, returning error for Codex to fix"));
...
bool FScriptExecutionManager::TriggerLiveCodingCompile(FString& OutErrorLog)
{
	...
	if (OutputCapture.bHasErrors)
	{
		...
		UE_LOG(LogUnrealAIAssistant, Error, TEXT("Live Coding compilation failed:\n%s"), *OutErrorLog);
		return false;
	}

	UE_LOG(LogUnrealAIAssistant, Log, TEXT("Live Coding compilation completed successfully"));
	return true;
#else
	...
	const FString& Description)
{
	...
	FUnrealAIAssistantOutputDevice ExecOutput;
	FUnrealAIAssistantOutputDevice LogOutput;
	...
	UE_LOG(LogUnrealAIAssistant, Log, TEXT("Python script output (%d chars): %s"),
		Output.Len(), Output.Len() > 500 ? *(Output.Left(500) + TEXT("...")) : *Output);
	UE_LOG(LogUnrealAIAssistant, Log, TEXT("Python script actor delta: %d before, %d after (%+d)"),
	...
	// Append output to result so Codex can see what happened
	...
	const FString& Description)
{
	...
	FUnrealAIAssistantOutputDevice OutputDevice;
	FString AllOutput;
	...
	for (const FString& RawCommand : Commands)
	{
		...
		if (Command.Contains(TEXT("@UnrealAIAssistant")) || Command.Contains(TEXT("@Name:")) ||
	...
	const FString& ScriptName)
{
	...
	UE_LOG(LogUnrealAIAssistant, Error, TEXT("Failed to write script to: %s"), *FilePath);
	return FString();
}
...
bool FScriptExecutionManager::SaveHistory()
{
	...
	{
		UE_LOG(LogUnrealAIAssistant, Log, TEXT("Script history saved: %d entries"), History.Num());
		return true;
	}

	UE_LOG(LogUnrealAIAssistant, Error, TEXT("Failed to save script history to: %s"), *HistoryPath);
	return false;
}
...
bool FScriptExecutionManager::LoadHistory()
{
	...
	{
		UE_LOG(LogUnrealAIAssistant, Log, TEXT("No script history file found"));
		return false;
	}
	...
	{
		UE_LOG(LogUnrealAIAssistant, Error, TEXT("Failed to load script history from: %s"), *HistoryPath);
		return false;
	}
	...
	{
		UE_LOG(LogUnrealAIAssistant, Error, TEXT("Failed to parse script history JSON"));
		return false;
	}
	...
	UE_LOG(LogUnrealAIAssistant, Log, TEXT("Loaded script history: %d entries"), History.Num());
	return true;
}
...
{
	return FPaths::Combine(FPaths::ProjectSavedDir(), TEXT("UnrealAIAssistant"), TEXT("script_history.json"));
}

FString FScriptExecutionManager::GetCppScriptDirectory() const
{
	// Source/[ProjectName]/Generated/UnrealAIAssistant/
	FString ProjectName = FApp::GetProjectName();
	return FPaths::Combine(FPaths::ProjectDir(), TEXT("Source"), ProjectName, TEXT("Generated"), TEXT("UnrealAIAssistant"));
}

FString FScriptExecutionManager::GetContentScriptDirectory() const
{
	// Content/UnrealAIAssistant/Scripts/
	return FPaths::Combine(FPaths::ProjectContentDir(), TEXT("UnrealAIAssistant"), TEXT("Scripts"));
}
