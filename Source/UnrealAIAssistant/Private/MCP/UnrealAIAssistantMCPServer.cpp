...
#include "UnrealAIAssistantMCPServer.h"
...
FUnrealAIAssistantMCPServer::FUnrealAIAssistantMCPServer()
	: bIsRunning(false)
	, ServerPort(UnrealAIAssistantConstants::MCPServer::DefaultPort)
{
...
bool FUnrealAIAssistantMCPServer::Start(uint32 Port)
{
	if (bIsRunning)
	{
		UE_LOG(LogUnrealAIAssistant, Warning, TEXT("MCP Server is already running on port %d"), ServerPort);
		return true;
	}
	...
	{
		UE_LOG(LogUnrealAIAssistant, Error, TEXT("Failed to get HTTP router for port %d"), ServerPort);
		return false;
	}
	...
	UE_LOG(LogUnrealAIAssistant, Log, TEXT("MCP Server started on http://localhost:%d"), ServerPort);
	UE_LOG(LogUnrealAIAssistant, Log, TEXT("  GET  /mcp/tools      - List available tools"));
	UE_LOG(LogUnrealAIAssistant, Log, TEXT("  POST /mcp/tool/{name} - Execute a tool"));
	UE_LOG(LogUnrealAIAssistant, Log, TEXT("  GET  /mcp/status     - Server status"));

	return true;
}

void FUnrealAIAssistantMCPServer::Stop()
{
	if (!bIsRunning)
	{
		return;
	...
	bIsRunning = false;
	UE_LOG(LogUnrealAIAssistant, Log, TEXT("MCP Server stopped"));
}

void FUnrealAIAssistantMCPServer::SetupRoutes()
{
	if (!HttpRouter.IsValid())
	{
		return;
	...
	ListToolsHandle = HttpRouter->BindRoute(
		...
		FHttpRequestHandler::CreateRaw(this, &FUnrealAIAssistantMCPServer::HandleListTools)
	);
	...
	ExecuteToolHandle = HttpRouter->BindRoute(
		...
		FHttpRequestHandler::CreateRaw(this, &FUnrealAIAssistantMCPServer::HandleExecuteTool)
	);
	...
	StatusHandle = HttpRouter->BindRoute(
		...
		EHttpServerRequestVerbs::VERB_GET,
		FHttpRequestHandler::CreateRaw(this, &FUnrealAIAssistantMCPServer::HandleStatus)
	);
}

bool FUnrealAIAssistantMCPServer::HandleListTools(const FHttpServerRequest& Request, const FHttpResultCallback& OnComplete)
{
...
bool FUnrealAIAssistantMCPServer::HandleExecuteTool(const FHttpServerRequest& Request, const FHttpResultCallback& OnComplete)
{
	...
	if (Request.Body.Num() > UnrealAIAssistantConstants::MCPServer::MaxRequestBodySize)
	{
		UE_LOG(LogUnrealAIAssistant, Warning, TEXT("Request body too large: %d bytes (max %d)"), Request.Body.Num(), UnrealAIAssistantConstants::MCPServer::MaxRequestBodySize);
	...
	if (Request.Body.Num() > 0)
	{
		...
		{
			UE_LOG(LogUnrealAIAssistant, Warning, TEXT("Failed to parse JSON body: %s"), *BodyString);
...
bool FUnrealAIAssistantMCPServer::HandleStatus(const FHttpServerRequest& Request, const FHttpResultCallback& OnComplete)
{
	TSharedPtr<FJsonObject> ResponseJson = MakeShared<FJsonObject>();
...
TUniquePtr<FHttpServerResponse> FUnrealAIAssistantMCPServer::CreateJsonResponse(const FString& JsonContent, EHttpServerResponseCodes Code)
{
...
TUniquePtr<FHttpServerResponse> FUnrealAIAssistantMCPServer::CreateErrorResponse(const FString& Message, EHttpServerResponseCodes Code)
{
