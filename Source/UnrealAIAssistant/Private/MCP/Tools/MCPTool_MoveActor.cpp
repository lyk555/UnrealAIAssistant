...
FMCPToolResult FMCPTool_MoveActor::Execute(const TSharedRef<FJsonObject>& Params)
{
	...
	ResultData->SetObjectField(TEXT("location"), UnrealAIAssistantJsonUtils::VectorToJson(Actor->GetActorLocation()));
	ResultData->SetObjectField(TEXT("rotation"), UnrealAIAssistantJsonUtils::RotatorToJson(Actor->GetActorRotation()));
	ResultData->SetObjectField(TEXT("scale"), UnrealAIAssistantJsonUtils::VectorToJson(Actor->GetActorScale3D()));
