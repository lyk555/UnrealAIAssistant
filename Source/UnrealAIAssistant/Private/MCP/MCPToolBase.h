...
protected:
		...
		const FVector& DefaultValue = FVector::ZeroVector) const
	{
		...
		{
			return UnrealAIAssistantJsonUtils::ExtractVector(*VectorObj, DefaultValue);
		}
		return DefaultValue;
	}
		...
		const FRotator& DefaultValue = FRotator::ZeroRotator) const
	{
		...
		{
			return UnrealAIAssistantJsonUtils::ExtractRotator(*RotatorObj, DefaultValue);
		}
		return DefaultValue;
	}
	...
	TSharedPtr<FJsonObject> BuildActorInfoWithTransformJson(AActor* Actor) const
	{
		...
		if (Actor)
		{
			ActorJson->SetObjectField(TEXT("location"), UnrealAIAssistantJsonUtils::VectorToJson(Actor->GetActorLocation()));
			ActorJson->SetObjectField(TEXT("rotation"), UnrealAIAssistantJsonUtils::RotatorToJson(Actor->GetActorRotation()));
			ActorJson->SetObjectField(TEXT("scale"), UnrealAIAssistantJsonUtils::VectorToJson(Actor->GetActorScale3D()));
		}
		return ActorJson;
	}
