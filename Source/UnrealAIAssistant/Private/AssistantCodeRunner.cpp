...
{
	const FString EnginePluginPath = FPaths::Combine(FPaths::EnginePluginsDir(), TEXT("UnrealAIAssistant"));
	if (FPaths::DirectoryExists(EnginePluginPath))
	{
		return EnginePluginPath;
	}

	const FString MarketplacePluginPath = FPaths::Combine(FPaths::EnginePluginsDir(), TEXT("Marketplace"), TEXT("UnrealAIAssistant"));
	if (FPaths::DirectoryExists(MarketplacePluginPath))
	{
		return MarketplacePluginPath;
	}

	const FString ProjectPluginPath = FPaths::Combine(FPaths::ProjectPluginsDir(), TEXT("UnrealAIAssistant"));
	if (FPaths::DirectoryExists(ProjectPluginPath))
	{
		return ProjectPluginPath;
	}

	UE_LOG(LogUnrealAIAssistant, Warning, TEXT("Could not find UnrealAIAssistant plugin directory."));
	return FString();
}
...
void CopyCodexHomeFileIfPresent(const FString& SourceHomePath, const FString& TargetHomePath, const FString& FileName)
{
	...
	{
		UE_LOG(LogUnrealAIAssistant, Verbose, TEXT("Failed to copy Codex home file: %s"), *SourcePath);
	}
}
...
bool WriteMcpConfigToml(const FString& CodexHomePath, const FString& BridgePath)
{
	...
	{
		UE_LOG(LogUnrealAIAssistant, Warning, TEXT("Failed to write Codex config.toml to: %s"), *ConfigPath);
		return false;
	}

	UE_LOG(LogUnrealAIAssistant, Log, TEXT("Codex MCP config written to: %s"), *ConfigPath);
	return true;
}
}

FAssistantCodeRunner::FAssistantCodeRunner()
	: Thread(nullptr)
	, bIsExecuting(false)
	, ReadPipe(nullptr)
	, WritePipe(nullptr)
...
void FAssistantCodeRunner::CleanupHandles()
{
...
bool FAssistantCodeRunner::IsCodexAvailable()
{
	return !GetCodexPath().IsEmpty();
}

FString FAssistantCodeRunner::GetCodexPath()
{
	...
	for (const FString& Path : PossiblePaths)
	{
		if (IFileManager::Get().FileExists(*Path))
		{
			...
			UE_LOG(LogUnrealAIAssistant, Log, TEXT("Found Codex CLI at: %s"), *CachedCodexPath);
			return CachedCodexPath;
		}
	}
	...
	if (FPlatformProcess::ExecProcess(WhichCmd, WhichArgs, &ReturnCode, &WhereOutput, &WhereErrors) && ReturnCode == 0)
	{
		...
		if (Lines.Num() > 0)
		{
			...
			UE_LOG(LogUnrealAIAssistant, Log, TEXT("Found Codex CLI via lookup: %s"), *CachedCodexPath);
			return CachedCodexPath;
		}
	}

	UE_LOG(LogUnrealAIAssistant, Warning, TEXT("Codex CLI not found. Install the Codex CLI, then run `codex login`."));
	return CachedCodexPath;
}

bool FAssistantCodeRunner::ExecuteAsync(
	...
	FOnCodexProgress OnProgress)
{
	...
	{
		UE_LOG(LogUnrealAIAssistant, Warning, TEXT("Codex is already executing a request"));
		return false;
	}
	...
	Thread = FRunnableThread::Create(this, TEXT("AssistantCodeRunner"), 0, TPri_Normal);
	if (!Thread)
	{
		bIsExecuting = false;
		return false;
...
bool FAssistantCodeRunner::ExecuteSync(const FCodexRequestConfig& Config, FString& OutResponse)
{
	OutResponse = TEXT("Synchronous Codex execution is not supported by UnrealAIAssistant. Use ExecuteAsync instead.");
	return false;
}

FString FAssistantCodeRunner::BuildCommandLine(const FCodexRequestConfig& Config)
{
	...
	if (!Config.ModelId.IsEmpty())
	{
		...
		CommandLine += FString::Printf(TEXT("-m \"%s\" "), *EscapedModelId);
	}
	...
	const FString TempDir = FPaths::Combine(FPaths::ProjectSavedDir(), TEXT("UnrealAIAssistant"));
	...
	if (!PluginDir.IsEmpty())
	{
		...
		{
			UE_LOG(LogUnrealAIAssistant, Warning, TEXT("MCP bridge not found at: %s"), *BridgePath);
		}
	}
...
FString FAssistantCodeRunner::BuildPromptText(const FCodexRequestConfig& Config) const
{
...
FString FAssistantCodeRunner::BuildStreamJsonPayload(const FString& TextPrompt, const TArray<FString>& ImagePaths)
{
	...
	JsonLine += TEXT("\n");
	UE_LOG(LogUnrealAIAssistant, Verbose, TEXT("Built legacy image payload: %d chars (%d images)"), JsonLine.Len(), EncodedCount);
	return JsonLine;
}

FString FAssistantCodeRunner::ParseStreamJsonOutput(const FString& RawOutput)
{
	TArray<FString> Lines;
	...
	UE_LOG(LogUnrealAIAssistant, Warning, TEXT("Failed to parse Codex JSONL output (%d chars)."), RawOutput.Len());
...
void FAssistantCodeRunner::ParseAndEmitJsonlLine(const FString& JsonLine)
{
	if (JsonLine.IsEmpty())
	{
		return;
	...
	if (!FJsonSerializer::Deserialize(Reader, JsonObj) || !JsonObj.IsValid())
	{
		UE_LOG(LogUnrealAIAssistant, Verbose, TEXT("JSONL: Non-JSON line (skipping): %.200s"), *JsonLine);
		return;
	}
	...
	if (Type == TEXT("thread.started"))
	{
		if (CurrentConfig.OnStreamEvent.IsBound())
		{
			...
			Event.Provider = ECodexProvider::Codex;
	...
	if (Type == TEXT("error"))
	{
		...
		UE_LOG(LogUnrealAIAssistant, Warning, TEXT("Codex stream warning: %s"), *ErrorText.Left(1000));
	}
}

void FAssistantCodeRunner::Cancel()
{
	StopTaskCounter.Set(1);
	if (ProcessHandle.IsValid())
	{
...
bool FAssistantCodeRunner::Init()
{
...
uint32 FAssistantCodeRunner::Run()
{
	ExecuteProcess();
	return 0;
}

void FAssistantCodeRunner::Stop()
{
	StopTaskCounter.Increment();
}

void FAssistantCodeRunner::Exit()
{
	bIsExecuting = false;
}

bool FAssistantCodeRunner::CreateProcessPipes()
{
...
bool FAssistantCodeRunner::LaunchProcess(const FString& FullCommand, const FString& WorkingDir)
{
	...
	if (!ProcessHandle.IsValid())
	{
		UE_LOG(LogUnrealAIAssistant, Error, TEXT("Failed to create Codex process"));
		UE_LOG(LogUnrealAIAssistant, Error, TEXT("Codex Path: %s"), *CodexPath);
		UE_LOG(LogUnrealAIAssistant, Error, TEXT("Executable: %s"), *ExecutablePath);
		UE_LOG(LogUnrealAIAssistant, Error, TEXT("Params: %s"), *ProcessArgs);
...
FString FAssistantCodeRunner::ReadProcessOutput()
{
	FString FullOutput;
	JsonlLineBuffer.Empty();
	AccumulatedResponseText.Empty();
...
void FAssistantCodeRunner::ReportError(const FString& ErrorMessage)
{
...
void FAssistantCodeRunner::ReportCompletion(const FString& Output, bool bSuccess)
{
	FOnCodexResponse CompleteCopy = OnCompleteDelegate;
...
void FAssistantCodeRunner::ExecuteProcess()
{
	const FString CodexPath = GetCodexPath();
...
// FCodexCodeSubsystem is now in AssistantSubsystem.cpp
