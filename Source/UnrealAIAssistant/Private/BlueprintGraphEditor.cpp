	...
	FString& OutError)
{
	...
	// Get the appropriate graph array (UE 5.5 uses TObjectPtr)
	...
	FString& OutError)
{
	...
	if (NewNode)
	{
		...
		UE_LOG(LogUnrealAIAssistant, Log, TEXT("Created node '%s' (type: %s) at (%d, %d)"), *OutNodeId, *NodeType, PosX, PosY);
	}
...
bool FBlueprintGraphEditor::DeleteNode(UEdGraph* Graph, const FString& NodeId, FString& OutError)
{
	...
	UE_LOG(LogUnrealAIAssistant, Log, TEXT("Deleted node '%s'"), *NodeId);
	return true;
}
	...
	FString& OutError)
{
	...
	UE_LOG(LogUnrealAIAssistant, Log, TEXT("Connected '%s.%s' -> '%s.%s'"),
	...
	FString& OutError)
{
	...
	UE_LOG(LogUnrealAIAssistant, Log, TEXT("Disconnected '%s.%s' from '%s.%s'"),
	...
	FString& OutError)
{
	...
	UE_LOG(LogUnrealAIAssistant, Log, TEXT("Set pin '%s.%s' default value to '%s'"), *NodeId, *PinName, *Value);
	return true;
}
...
TSharedPtr<FJsonObject> FBlueprintGraphEditor::SerializeNodeInfo(UEdGraphNode* Node)
{
	...
	for (UEdGraphPin* Pin : Node->Pins)
	{
		...
		for (UEdGraphPin* LinkedPin : Pin->LinkedTo)
		{
			...
			LinkObj->SetStringField(TEXT("node_title"), LinkedPin->GetOwningNode()->GetNodeTitle(ENodeTitleType::ListView).ToString());
			LinkedToArray.Add(MakeShared<FJsonValueObject>(LinkObj));
		}
		if (LinkedToArray.Num() > 0)
		{
...
FString FBlueprintGraphEditor::GetNodeId(UEdGraphNode* Node)
{
	...
	if (Node->NodeGuid.IsValid())
	{
		return FString::Printf(TEXT("guid_%s"), *Node->NodeGuid.ToString(EGuidFormats::Digits));
	}
