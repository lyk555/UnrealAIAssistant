...
FString FClaudeCodeRunner::GetClaudePath()
{
	...
	for (const FString& Path : PossiblePaths)
	{
		...
		{
			UE_LOG(LogUnrealAIAssistant, Log, TEXT("Found Claude CLI at: %s"), *Path);
	...
	if (FPlatformProcess::ExecProcess(WhichCmd, WhichArgs, &ReturnCode, &WhereOutput, &WhereErrors) && ReturnCode == 0)
	{
		...
		if (Lines.Num() > 0)
		{
			UE_LOG(LogUnrealAIAssistant, Log, TEXT("Found Claude CLI via '%s': %s"), WhichCmd, *Lines[0]);
			CachedClaudePath = Lines[0];
			return CachedClaudePath;
		}
	}

	UE_LOG(LogUnrealAIAssistant, Warning, TEXT("Claude CLI not found. Please install with: npm install -g @anthropic-ai/claude-code"));
	...
	FOnCodexProgress OnProgress)
{
	...
	{
		UE_LOG(LogUnrealAIAssistant, Warning, TEXT("Claude is already executing a request"));
		return false;
	}
...
bool FClaudeCodeRunner::ExecuteSync(const FCodexRequestConfig& Config, FString& OutResponse)
{
	if (!IsClaudeAvailable())
	{
	...
	UE_LOG(LogUnrealAIAssistant, Log, TEXT("Executing Claude: %s %s"), *ClaudePath, *CommandLine);
	...
	else
	{
		...
		UE_LOG(LogUnrealAIAssistant, Error, TEXT("Claude execution failed: %s"), *OutResponse);
		return false;
	}
}
...
static FString GetPluginDirectory()
{
	...
	FString EnginePluginPath = FPaths::Combine(FPaths::EnginePluginsDir(), TEXT("UnrealAIAssistant"));
	...
	FString MarketplacePluginPath = FPaths::Combine(FPaths::EnginePluginsDir(), TEXT("Marketplace"), TEXT("UnrealAIAssistant"));
	...
	// Try project plugins
	FString ProjectPluginPath = FPaths::Combine(FPaths::ProjectPluginsDir(), TEXT("UnrealAIAssistant"));
	if (FPaths::DirectoryExists(ProjectPluginPath))
	{
		return ProjectPluginPath;
	}

	UE_LOG(LogUnrealAIAssistant, Warning, TEXT("Could not find UnrealAIAssistant plugin directory for Claude provider. Checked: %s, %s, %s"),
		*EnginePluginPath, *MarketplacePluginPath, *ProjectPluginPath);
	return FString();
}

FString FClaudeCodeRunner::BuildCommandLine(const FCodexRequestConfig& Config)
{
	FString CommandLine;
	...
	{
		FString EscapedModelId = Config.ModelId.Replace(TEXT("\""), TEXT("\\\""));
		CommandLine += FString::Printf(TEXT("--model \"%s\" "), *EscapedModelId);
	}
	...
	if (!PluginDir.IsEmpty())
	{
		...
		if (FPaths::FileExists(MCPBridgePath))
		{
			...
			FString MCPConfigDir = FPaths::Combine(FPaths::ProjectSavedDir(), TEXT("UnrealAIAssistant"));
			...
			if (FFileHelper::SaveStringToFile(MCPConfigContent, *MCPConfigPath))
			{
				...
				UE_LOG(LogUnrealAIAssistant, Log, TEXT("MCP config written to: %s"), *MCPConfigPath);
			}
			else
			{
				UE_LOG(LogUnrealAIAssistant, Warning, TEXT("Failed to write MCP config to: %s"), *MCPConfigPath);
			}
		}
		else
		{
			UE_LOG(LogUnrealAIAssistant, Warning, TEXT("MCP bridge not found at: %s"), *MCPBridgePath);
		}
	}
	...
	FString TempDir = FPaths::Combine(FPaths::ProjectSavedDir(), TEXT("UnrealAIAssistant"));
	...
	if (!Config.SystemPrompt.IsEmpty())
	{
		...
		if (FFileHelper::SaveStringToFile(Config.SystemPrompt, *SystemPromptPath, FFileHelper::EEncodingOptions::ForceUTF8WithoutBOM))
		{
			...
			UE_LOG(LogUnrealAIAssistant, Log, TEXT("System prompt written to: %s (%d chars)"), *SystemPromptPath, Config.SystemPrompt.Len());
		}
	}
	...
	if (FFileHelper::SaveStringToFile(Config.Prompt, *PromptPath, FFileHelper::EEncodingOptions::ForceUTF8WithoutBOM))
	{
		...
		UE_LOG(LogUnrealAIAssistant, Log, TEXT("Prompt written to: %s (%d chars)"), *PromptPath, Config.Prompt.Len());
	}
...
{
	using namespace UnrealAIAssistantConstants::ClipboardImage;

	// Pre-compute expected directory once for all images
	FString ExpectedDir = FPaths::ConvertRelativePathToFull(
		FPaths::Combine(FPaths::ProjectSavedDir(), TEXT("UnrealAIAssistant"), TEXT("screenshots")));
	...
	for (int32 i = 0; i < MaxCount; ++i)
	{
		...
		{
			UE_LOG(LogUnrealAIAssistant, Warning, TEXT("Rejecting image path with traversal: %s"), *FullImagePath);
			continue;
		}
		if (!FullImagePath.StartsWith(ExpectedDir))
		{
			UE_LOG(LogUnrealAIAssistant, Warning, TEXT("Rejecting image path outside screenshots directory: %s"), *FullImagePath);
			continue;
		}
		if (!FPaths::FileExists(FullImagePath))
		{
			UE_LOG(LogUnrealAIAssistant, Warning, TEXT("Attached image file no longer exists: %s"), *FullImagePath);
			continue;
		}
		...
		{
			UE_LOG(LogUnrealAIAssistant, Warning, TEXT("Image file too large for base64 encoding: %s (%lld bytes, max %lld)"),
		...
		{
			UE_LOG(LogUnrealAIAssistant, Warning, TEXT("Skipping image (total payload would exceed %lld bytes): %s"),
		...
		{
			UE_LOG(LogUnrealAIAssistant, Warning, TEXT("Failed to load image file for base64 encoding: %s"), *FullImagePath);
			continue;
		}
		...
		EncodedCount++;
		UE_LOG(LogUnrealAIAssistant, Log, TEXT("Base64 encoded image [%d]: %s (%d bytes -> %d chars)"),
			EncodedCount, *FullImagePath, ImageData.Num(), Base64ImageData.Len());
	}

	if (EncodedCount > 0)
	{
		UE_LOG(LogUnrealAIAssistant, Log, TEXT("Encoded %d image(s), total %lld bytes"), EncodedCount, TotalImageBytes);
	}
	...
	UE_LOG(LogUnrealAIAssistant, Log, TEXT("Built stream-json payload: %d chars (images: %d)"),
...
FString FClaudeCodeRunner::ParseStreamJsonOutput(const FString& RawOutput)
{
	...
	for (const FString& Line : Lines)
	{
		...
		if (Type == TEXT("result"))
		{
			...
			{
				UE_LOG(LogUnrealAIAssistant, Log, TEXT("Parsed stream-json result: %d chars"), ResultText.Len());
				return ResultText;
			}
		}
	}
	...
	{
		UE_LOG(LogUnrealAIAssistant, Log, TEXT("Parsed stream-json from assistant blocks: %d chars"), AccumulatedText.Len());
		return AccumulatedText;
	}

	// Last resort: return a user-friendly error instead of raw NDJSON
	UE_LOG(LogUnrealAIAssistant, Warning, TEXT("Failed to parse stream-json output (%d chars). Raw output logged below:"), RawOutput.Len());
	UE_LOG(LogUnrealAIAssistant, Warning, TEXT("%s"), *RawOutput.Left(2000));
	return TEXT("Error: Failed to parse the Claude provider response. Check the Output Log for details.");
}
...
void FClaudeCodeRunner::ParseAndEmitNdjsonLine(const FString& JsonLine)
{
	...
	if (!FJsonSerializer::Deserialize(Reader, JsonObj) || !JsonObj.IsValid())
	{
		UE_LOG(LogUnrealAIAssistant, Verbose, TEXT("NDJSON: Non-JSON line (skipping): %.200s"), *JsonLine);
		return;
	}
	...
	if (!JsonObj->TryGetStringField(TEXT("type"), Type))
	{
		UE_LOG(LogUnrealAIAssistant, Verbose, TEXT("NDJSON: Line missing 'type' field"));
		return;
	}

	UE_LOG(LogUnrealAIAssistant, Log, TEXT("NDJSON Event: type=%s"), *Type);
	...
	if (Type == TEXT("system"))
	{
		...
		UE_LOG(LogUnrealAIAssistant, Log, TEXT("NDJSON System: subtype=%s, session_id=%s"), *Subtype, *SessionId);

		if (CurrentConfig.OnStreamEvent.IsBound())
		{
			FCodexStreamEvent Event;
			Event.Type = ECodexStreamEventType::SessionInit;
			Event.Provider = ECodexProvider::Claude;
	...
	else if (Type == TEXT("assistant"))
	{
		...
		if (!JsonObj->TryGetObjectField(TEXT("message"), MessageObj))
		{
			UE_LOG(LogUnrealAIAssistant, Warning, TEXT("NDJSON: assistant message missing 'message' field"));
			return;
		}
		...
		if (!(*MessageObj)->TryGetArrayField(TEXT("content"), ContentArray))
		{
			UE_LOG(LogUnrealAIAssistant, Warning, TEXT("NDJSON: assistant message.content missing"));
			return;
		}
		...
		for (const TSharedPtr<FJsonValue>& ContentValue : *ContentArray)
		{
			...
			if (ContentType == TEXT("text"))
			{
				...
				if ((*ContentObj)->TryGetStringField(TEXT("text"), Text))
				{
					UE_LOG(LogUnrealAIAssistant, Log, TEXT("NDJSON TextContent: %d chars"), Text.Len());
			...
			else if (ContentType == TEXT("tool_use"))
			{
				...
				UE_LOG(LogUnrealAIAssistant, Log, TEXT("NDJSON ToolUse: name=%s, id=%s, input=%d chars"),
			...
			{
				UE_LOG(LogUnrealAIAssistant, Verbose, TEXT("NDJSON: unknown content block type: %s"), *ContentType);
			}
		}
	}
	else if (Type == TEXT("user"))
	{
		...
		for (const TSharedPtr<FJsonValue>& ContentValue : *ContentArray)
		{
			...
			if (ContentType == TEXT("tool_result"))
			{
				...
				UE_LOG(LogUnrealAIAssistant, Log, TEXT("NDJSON ToolResult: tool_use_id=%s, content=%d chars"),
	...
	else if (Type == TEXT("result"))
	{
		...
		UE_LOG(LogUnrealAIAssistant, Log, TEXT("NDJSON Result: subtype=%s, is_error=%d, duration=%.0fms, turns=%.0f, cost=$%.4f, result=%d chars"),
	...
	{
		UE_LOG(LogUnrealAIAssistant, Verbose, TEXT("NDJSON: unhandled message type: %s"), *Type);
	}
}
...
bool FClaudeCodeRunner::CreateProcessPipes()
{
	...
	{
		UE_LOG(LogUnrealAIAssistant, Error, TEXT("Failed to create stdout pipe"));
		return false;
	}
	...
	{
		UE_LOG(LogUnrealAIAssistant, Error, TEXT("Failed to create stdin pipe"));
		FPlatformProcess::ClosePipe(ReadPipe, WritePipe);
		ReadPipe = nullptr;
		WritePipe = nullptr;
		return false;
...
bool FClaudeCodeRunner::LaunchProcess(const FString& FullCommand, const FString& WorkingDir)
{
	...
	if (!ProcessHandle.IsValid())
	{
		UE_LOG(LogUnrealAIAssistant, Error, TEXT("Failed to create Claude process"));
		UE_LOG(LogUnrealAIAssistant, Error, TEXT("Claude Path: %s"), *ClaudePath);
		UE_LOG(LogUnrealAIAssistant, Error, TEXT("Params: %s"), *Params);
		UE_LOG(LogUnrealAIAssistant, Error, TEXT("Working directory: %s"), *WorkingDir);
		return false;
	}
...
void FClaudeCodeRunner::ExecuteProcess()
{
	...
	{
		UE_LOG(LogUnrealAIAssistant, Error, TEXT("Claude path no longer exists: %s"), *ClaudePath);
	...
	UE_LOG(LogUnrealAIAssistant, Log, TEXT("Async executing Claude: %s %s"), *ClaudePath, *CommandLine);
	...
	UE_LOG(LogUnrealAIAssistant, Log, TEXT("Full command: %s %s"), *ClaudePath, *CommandLine);
	UE_LOG(LogUnrealAIAssistant, Log, TEXT("Working directory: %s"), *WorkingDir);
	...
	if (StdInWritePipe)
	{
		...
		if (!StdinPayload.IsEmpty())
		{
			...
			UE_LOG(LogUnrealAIAssistant, Log, TEXT("Wrote to Claude stdin (stream-json, success=%d, %d/%d bytes, images: %d, system: %d chars, user: %d chars)"),
	...
	if (ResponseText.IsEmpty() && !FullOutput.IsEmpty())
	{
		...
		UE_LOG(LogUnrealAIAssistant, Log, TEXT("NDJSON parser produced no text, fell back to legacy parser (%d chars)"),
			ResponseText.Len());
	}
...
// Claude provider runner lives inside UnrealAIAssistant.
