...
void SAssistantInputArea::Construct(const FArguments& InArgs)
{
		...
		[
			...
			[
				...
				[
					...
					[
						...
						.HintText(LOCTEXT("InputHint", "Ask assistant about Unreal Engine 5.5... (Shift+Enter for newline)"))
						.AutoWrapText(true)
						.AllowMultiLine(true)
						.OnTextChanged(this, &SAssistantInputArea::HandleTextChanged)
						.OnTextCommitted(this, &SAssistantInputArea::HandleTextCommitted)
						.OnKeyDownHandler(this, &SAssistantInputArea::OnInputKeyDown)
			...
			[
				...
				[
					...
					.OnClicked(this, &SAssistantInputArea::HandlePasteClicked)
				...
				[
					...
					.OnClicked(this, &SAssistantInputArea::HandleSendCancelClicked)
...
void SAssistantInputArea::SetText(const FString& NewText)
{
...
FString SAssistantInputArea::GetText() const
{
	return CurrentInputText;
}

void SAssistantInputArea::ClearText()
{
...
bool SAssistantInputArea::HasAttachedImages() const
{
	return AttachedImagePaths.Num() > 0;
}

int32 SAssistantInputArea::GetAttachedImageCount() const
{
	return AttachedImagePaths.Num();
}

TArray<FString> SAssistantInputArea::GetAttachedImagePaths() const
{
	return AttachedImagePaths;
}

void SAssistantInputArea::ClearAttachedImages()
{
...
void SAssistantInputArea::RemoveAttachedImage(int32 Index)
{
...
FReply SAssistantInputArea::OnInputKeyDown(const FGeometry& MyGeometry, const FKeyEvent& InKeyEvent)
{
...
void SAssistantInputArea::HandleTextChanged(const FText& NewText)
{
	CurrentInputText = NewText.ToString();
	OnTextChangedDelegate.ExecuteIfBound(CurrentInputText);
}

void SAssistantInputArea::HandleTextCommitted(const FText& NewText, ETextCommit::Type CommitType)
{
	// Don't send on commit - use explicit Enter key handling
}

FReply SAssistantInputArea::HandlePasteClicked()
{
...
FReply SAssistantInputArea::HandleSendCancelClicked()
{
	if (bIsWaiting.Get())
	{
...
bool SAssistantInputArea::TryPasteImageFromClipboard()
{
	...
	{
		UE_LOG(LogUnrealAIAssistant, Log, TEXT("Image paste rejected: already at max (%d images)"), MaxImagesPerMessage);
		return false;
	}
	...
	FClipboardImageUtils::CleanupOldScreenshots(
		ScreenshotDir,
		UnrealAIAssistantConstants::ClipboardImage::MaxScreenshotAgeSeconds);
...
FReply SAssistantInputArea::HandleRemoveImageClicked(int32 Index)
{
	RemoveAttachedImage(Index);
	return FReply::Handled();
}

void SAssistantInputArea::RebuildImagePreviewStrip()
{
	using namespace UnrealAIAssistantConstants::ClipboardImage;
...
TSharedPtr<FSlateDynamicImageBrush> SAssistantInputArea::CreateThumbnailBrush(const FString& FilePath) const
{
	// Load PNG file from disk
	TArray<uint8> PngData;
	if (!FFileHelper::LoadFileToArray(PngData, *FilePath))
	{
		UE_LOG(LogUnrealAIAssistant, Warning, TEXT("Failed to load image for thumbnail: %s"), *FilePath);
		return nullptr;
	}
	...
	{
		UE_LOG(LogUnrealAIAssistant, Warning, TEXT("Failed to decompress PNG for thumbnail"));
		return nullptr;
	}
	...
	{
		UE_LOG(LogUnrealAIAssistant, Warning, TEXT("Failed to get raw pixel data for thumbnail"));
		return nullptr;
	}
