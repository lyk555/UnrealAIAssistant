...
void SCodexInputArea::Construct(const FArguments& InArgs)
{
		...
		[
			...
			[
				...
				[
					...
					[
						...
						.HintText(LOCTEXT("InputHint", "Ask Codex about Unreal Engine 5.5... (Shift+Enter for newline)"))
						.AutoWrapText(true)
						.AllowMultiLine(true)
						.OnTextChanged(this, &SCodexInputArea::HandleTextChanged)
						.OnTextCommitted(this, &SCodexInputArea::HandleTextCommitted)
						.OnKeyDownHandler(this, &SCodexInputArea::OnInputKeyDown)
			...
			[
				...
				[
					...
					.OnClicked(this, &SCodexInputArea::HandlePasteClicked)
				...
				[
					...
					.OnClicked(this, &SCodexInputArea::HandleSendCancelClicked)
...
void SCodexInputArea::SetText(const FString& NewText)
{
...
FString SCodexInputArea::GetText() const
{
	return CurrentInputText;
}

void SCodexInputArea::ClearText()
{
...
bool SCodexInputArea::HasAttachedImages() const
{
	return AttachedImagePaths.Num() > 0;
}

int32 SCodexInputArea::GetAttachedImageCount() const
{
	return AttachedImagePaths.Num();
}

TArray<FString> SCodexInputArea::GetAttachedImagePaths() const
{
	return AttachedImagePaths;
}

void SCodexInputArea::ClearAttachedImages()
{
...
void SCodexInputArea::RemoveAttachedImage(int32 Index)
{
...
FReply SCodexInputArea::OnInputKeyDown(const FGeometry& MyGeometry, const FKeyEvent& InKeyEvent)
{
...
void SCodexInputArea::HandleTextChanged(const FText& NewText)
{
	CurrentInputText = NewText.ToString();
	OnTextChangedDelegate.ExecuteIfBound(CurrentInputText);
}

void SCodexInputArea::HandleTextCommitted(const FText& NewText, ETextCommit::Type CommitType)
{
	// Don't send on commit - use explicit Enter key handling
}

FReply SCodexInputArea::HandlePasteClicked()
{
...
FReply SCodexInputArea::HandleSendCancelClicked()
{
	if (bIsWaiting.Get())
	{
...
bool SCodexInputArea::TryPasteImageFromClipboard()
{
	...
	{
		UE_LOG(LogUnrealCodex, Log, TEXT("Image paste rejected: already at max (%d images)"), MaxImagesPerMessage);
		return false;
	}
	...
	FClipboardImageUtils::CleanupOldScreenshots(
		ScreenshotDir,
		UnrealCodexConstants::ClipboardImage::MaxScreenshotAgeSeconds);
...
FReply SCodexInputArea::HandleRemoveImageClicked(int32 Index)
{
	RemoveAttachedImage(Index);
	return FReply::Handled();
}

void SCodexInputArea::RebuildImagePreviewStrip()
{
	using namespace UnrealCodexConstants::ClipboardImage;
...
TSharedPtr<FSlateDynamicImageBrush> SCodexInputArea::CreateThumbnailBrush(const FString& FilePath) const
{
	// Load PNG file from disk
	TArray<uint8> PngData;
	if (!FFileHelper::LoadFileToArray(PngData, *FilePath))
	{
		UE_LOG(LogUnrealCodex, Warning, TEXT("Failed to load image for thumbnail: %s"), *FilePath);
		return nullptr;
	}
	...
	{
		UE_LOG(LogUnrealCodex, Warning, TEXT("Failed to decompress PNG for thumbnail"));
		return nullptr;
	}
	...
	{
		UE_LOG(LogUnrealCodex, Warning, TEXT("Failed to get raw pixel data for thumbnail"));
		return nullptr;
	}
