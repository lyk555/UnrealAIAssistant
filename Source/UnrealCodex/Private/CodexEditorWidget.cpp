...
#include "Widgets/SCodexToolbar.h"
#include "Widgets/SCodexInputArea.h"
...
void SCodexEditorWidget::Construct(const FArguments& InArgs)
{
	ChildSlot
	[
		SNew(SVerticalBox)
	...
	if (!IsCodexAvailable())
	{
		AddMessage(TEXT("Codex CLI not found.\n\nInstall the Codex CLI, then authenticate with:\n  codex login"), false);
	}
	else
	{
		FString WelcomeMessage = TEXT("👋 Welcome to Unreal Codex!\n\nI'm ready to help with your UE5.5 project. Ask me about:\n• C++ code patterns and best practices\n• Blueprint integration\n• Engine systems (Nanite, Lumen, GAS, etc.)\n• Debugging and optimization\n\n");
...
SCodexEditorWidget::~SCodexEditorWidget()
{
	// Cancel any pending requests
	FCodexCodeSubsystem::Get().CancelCurrentRequest();
}

TSharedRef<SWidget> SCodexEditorWidget::BuildToolbar()
{
	return SNew(SCodexToolbar)
...
TSharedRef<SWidget> SCodexEditorWidget::BuildChatArea()
{
	return SNew(SBorder)
		.BorderImage(FAppStyle::GetBrush("ToolPanel.DarkGroupBorder"))
		.Padding(4.0f)
...
TSharedRef<SWidget> SCodexEditorWidget::BuildInputArea()
{
	SAssignNew(InputArea, SCodexInputArea)
...
TSharedRef<SWidget> SCodexEditorWidget::BuildStatusBar()
{
	return SNew(SBorder)
		...
		[
			...
			[
				SNew(STextBlock)
				.Text(this, &SCodexEditorWidget::GetStatusText)
				.ColorAndOpacity(this, &SCodexEditorWidget::GetStatusColor)
			]
...
void SCodexEditorWidget::AddMessage(const FString& Message, bool bIsUser)
{
...
void SCodexEditorWidget::SendMessage()
{
	...
	OnComplete.BindSP(this, &SCodexEditorWidget::OnCodexResponse);
	...
	Options.OnProgress.BindSP(this, &SCodexEditorWidget::OnCodexProgress);
	Options.OnStreamEvent.BindSP(this, &SCodexEditorWidget::OnCodexStreamEvent);
	Options.AttachedImagePaths = ImagePaths;

	FCodexCodeSubsystem::Get().SendPrompt(Prompt, OnComplete, Options);
}

void SCodexEditorWidget::OnCodexResponse(const FString& Response, bool bSuccess)
{
...
void SCodexEditorWidget::ClearChat()
{
	if (ChatMessagesBox.IsValid())
	{
...
void SCodexEditorWidget::CancelRequest()
{
	FCodexCodeSubsystem::Get().CancelCurrentRequest();
	bIsWaitingForResponse = false;
...
void SCodexEditorWidget::CopyToClipboard()
{
	if (!LastResponse.IsEmpty())
	{
		FPlatformApplicationMisc::ClipboardCopy(*LastResponse);
		UE_LOG(LogUnrealCodex, Log, TEXT("Copied response to clipboard"));
	}
}

void SCodexEditorWidget::RestoreSession()
{
...
void SCodexEditorWidget::NewSession()
{
...
bool SCodexEditorWidget::IsCodexAvailable() const
{
	return FCodexCodeRunner::IsCodexAvailable();
}

FText SCodexEditorWidget::GetStatusText() const
{
...
FSlateColor SCodexEditorWidget::GetStatusColor() const
{
	if (bIsWaitingForResponse)
	{
...
void SCodexEditorWidget::ResetStreamingState()
{
	StreamingResponse.Empty();
...
void SCodexEditorWidget::StartStreamingResponse()
{
	ResetStreamingState();
...
void SCodexEditorWidget::OnCodexProgress(const FString& PartialOutput)
{
...
void SCodexEditorWidget::OnCodexStreamEvent(const FCodexStreamEvent& Event)
{
	switch (Event.Type)
	{
	case ECodexStreamEventType::SessionInit:
		UE_LOG(LogUnrealCodex, Log, TEXT("[StreamEvent] SessionInit: session_id=%s"), *Event.SessionId);
		break;

	case ECodexStreamEventType::TextContent:
		UE_LOG(LogUnrealCodex, Log, TEXT("[StreamEvent] TextContent: %d chars"), Event.Text.Len());
		break;

	case ECodexStreamEventType::ToolUse:
		UE_LOG(LogUnrealCodex, Log, TEXT("[StreamEvent] ToolUse: %s (id=%s)"), *Event.ToolName, *Event.ToolCallId);
		HandleToolUseEvent(Event);
		break;

	case ECodexStreamEventType::ToolResult:
		UE_LOG(LogUnrealCodex, Log, TEXT("[StreamEvent] ToolResult: tool_id=%s, %d chars"),
			*Event.ToolCallId, Event.ToolResultContent.Len());
		HandleToolResultEvent(Event);
		break;

	case ECodexStreamEventType::Result:
		UE_LOG(LogUnrealCodex, Log, TEXT("[StreamEvent] Result: error=%d, duration=%dms, turns=%d, cost=$%.4f"),
			Event.bIsError, Event.DurationMs, Event.NumTurns, Event.TotalCostUsd);
		HandleResultEvent(Event);
		break;

	default:
		UE_LOG(LogUnrealCodex, Log, TEXT("[StreamEvent] Unknown type: %d"), static_cast<int32>(Event.Type));
		break;
	}
}

void SCodexEditorWidget::FinalizeStreamingResponse()
{
...
void SCodexEditorWidget::HandleToolUseEvent(const FCodexStreamEvent& Event)
{
...
void SCodexEditorWidget::HandleToolResultEvent(const FCodexStreamEvent& Event)
{
	// Look up tool name
...
void SCodexEditorWidget::HandleResultEvent(const FCodexStreamEvent& Event)
{
	if (!StreamingContentBox.IsValid())
	{
		return;
...
FString SCodexEditorWidget::GetDisplayToolName(const FString& FullToolName)
{
...
void SCodexEditorWidget::UpdateToolGroupSummary()
{
...
void SCodexEditorWidget::ParseCodeFences(const FString& Input, TArray<TPair<FString, bool>>& OutSections)
{
	OutSections.Empty();
...
void SCodexEditorWidget::ParseAndRenderCodeBlocks()
{
...
void SCodexEditorWidget::AppendToLastResponse(const FString& Text)
{
	// Delegate to OnCodexProgress for streaming updates
	OnCodexProgress(Text);
}

void SCodexEditorWidget::RefreshProjectContext()
{
...
FText SCodexEditorWidget::GetProjectContextSummary() const
{
...
FString SCodexEditorWidget::GenerateMCPStatusMessage() const
{
	...
	if (!FUnrealCodexModule::IsAvailable())
	{
	...
	TSharedPtr<FUnrealCodexMCPServer> MCPServer = FUnrealCodexModule::Get().GetMCPServer();
	...
	const TArray<FString>& ExpectedTools = UnrealCodexConstants::MCPServer::ExpectedTools;
