...
// Dynamic context loader for UE 5.5 API documentation
import {
  getContextForTool,
  getContextForQuery,
  listCategories,
...
server.setRequestHandler(ListToolsRequestSchema, async () => {
  ...
  // 2. Expose the raw Unreal tool names directly.
  // Codex performs better when list_tools returns the same names referenced
  // by the editor prompt and by tool descriptions (spawn_actor, execute_script, task_submit, etc.).
  ...
  mcpTools.push({
    name: "get_ue_context",
    description: `Get UE 5.5 API context. Categories: ${listCategories().join(", ")}`,
    inputSchema: {
      type: "object",
      properties: {
        category: {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  if (name === "get_ue_context" || name === "unreal_get_ue_context") {
    ...
    return {
      content: [
        {
          type: "text",
          text: `# UE 5.5 Context: ${matchedCategories.join(", ")}\n\n${result}`,
        },
      ],
    };
  }
  ...
  if (name === "unreal_status") {
    ...
    if (status.connected) {
      ...
      const response = {
        ...
        exposed_tools: unrealTools.length + 2, // raw tools + status + context
  ...
  // Primary path: raw Unreal tool names as returned by list_tools.
  // Compatibility path: accept the legacy unreal_* prefix used by older bridge clients.
